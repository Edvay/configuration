...
##
cd /var/tmp
git clone https://github.com/Edvay/configuration.git
cd configuration
git checkout indus2
git pull
...
##
cd /var/tmp/configuration/playbooks/small_deployments && sudo -E ansible-playbook -c local ./edx-datastores.yml -i ./stateless.ini $EXTRA_VARS "$@"
